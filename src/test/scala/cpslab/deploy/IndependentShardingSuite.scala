...
import cpslab.deploy.utils.{Ping, DummyLSH}
...
class IndependentShardingSuite(var actorSystem: ActorSystem)
  ...
  test("(independent) Sharding scheme forwards search request correctly") {
    ...
    Thread.sleep(5000)
    assert(client.underlyingActor.state.contains("vector1") === true)
    assert(client.underlyingActor.state("vector1") === List(("vector0", 2.0)))
  }
}
