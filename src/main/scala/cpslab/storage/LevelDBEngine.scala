...
class LevelDBEngine extends KeyValueEngine {
  /**
   ...
   */
  override def put(key: Array[Byte], value: Array[Byte], allowOverwrite: Boolean): Boolean = ???
   ...
   * @return true -> write successfully; false -> something wrong happened during the write
   */
  override def write(keyValuePairs: List[(Array[Byte], Array[Byte])]): Boolean = ???
}
